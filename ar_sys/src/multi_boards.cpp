...
class ArSysMultiBoards
{
	private:
		...
		tf::TransformBroadcaster _tfBraodcaster;	// from phillip
	
		tf::Quaternion q1, q2;				// from phillip
	...
	public:
		...
		void image_callback(const sensor_msgs::ImageConstPtr& msg)
		{
			...
			try
			{
				...
				tf::Transform transform_ref = tf::Transform(q1*q2, tf::Vector3(0.13, 0.2, -0.05));		// from phillip
				tf::StampedTransform stampedTransform_ref(transform_ref, msg->header.stamp, "apollon_front", "apollon_cam_left");	//from phillip
				_tfBraodcaster.sendTransform(stampedTransform_ref);		// from phillip
				...
				for (int board_index = 0; board_index < boards.size(); board_index++)
				{
					...
					if (probDetect > 0.0)
					{
						...
						tf::StampedTransform stampedTransform(transform, msg->header.stamp, "apollon_cam_left", boards[board_index].name);
